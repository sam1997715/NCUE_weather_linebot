...
def handle_message_event(event, weatherbot, tokens):
    ...
    if message == "現在彰師大天氣":
        ...
        texts = "現在溫度: {}\n天氣狀態: {}\n濕度: {}\n紫外線指數: {}\n風速: {} \n風向: {}".format(results[0], results[1], results[2], results[3], results[4], results[5])
...
def convert_toemoji(desc):
    ...
    if len(sdesc) == 6:
        ...
        newDesc += "☂:氣象局未提供降雨機率\n"
    newDesc += sdesc[-2].replace(" ", "，", 1).replace("每秒", "").replace("公尺", "m/s") + "\n"
    newDesc += "濕度:" + sdesc[-1].replace("相對濕度", "") + "\n"
    newDesc += "---"
